        ...
        const TradingSimulator = () => {
            ...
            const searchStocks = async (query) => {
                ...
                try {
                    ...
                    if (results.length === 0) {
                        ...
                        if (results.length > 0) {
                            console.log(`✅ Found ${results.length} fuzzy matches:`, results.map(r => `${r.symbol} (${r.similarity.toFixed(0)}% match)`));
                        }
                    }
            ...
            const fetchStockData = async () => {
                ...
                try {
                    ...
                    if (stocksData.length > 0) {
                        ...
                        console.log('✅ All stocks loaded with Finnhub real-time prices:', stocksData);
                    }
            ...
            const fetchMarketMovers = async () => {
                ...
                try {
                    ...
                    console.log(`✅ Successfully fetched ${stocksData.length} stocks with valid data`);
            ...
            const addToWatchlist = async (symbol, stockName = null) => {
                ...
                if (plan === 'personal' && userEmail) {
                    ...
                    .then(() => console.log('✅ Watchlist synced to backend'))
            ...
            const createCompetition = async (memberName, contributionAmount, password = null) => {
                ...
                try {
                    ...
                    if (isPersonalPlan) {
                        ...
                        console.log('✅ Personal account created successfully:', code);
                        return;
                    }
            ...
            const handleQuantityChange = (newQuantity, isBuy = true) => {
                ...
                if (num > maxBuy) {
                    ...
                    setQuantityError(`⚠️ Insufficient funds! You need $${totalCost.toFixed(2)} but only have $${portfolio.cash.toFixed(2)}. Max: ${maxBuy} shares`);
                    setQuantity(num); // Still set it so they can see what they typed
                } else if (num > maxSell && !isBuy) {
                    setQuantityError(`⚠️ You only own ${maxSell} shares of ${selectedStock}`);
                    setQuantity(num);
                } else {
                    setQuantityError('');
                    setQuantity(Math.max(1, num));
            ...
            const buyMax = () => {
                ...
                if (maxShares === 0) {
                    setQuantityError('⚠️ Insufficient funds to buy any shares');
                    setQuantity(1);
                } else {
            ...
            const sellMax = () => {
                ...
                if (maxShares === 0) {
                    setQuantityError(`⚠️ You don't own any shares of ${selectedStock}`);
                    setQuantity(1);
                } else {
                    setQuantity(maxShares);
                    setQuantityError('');
            ...
            const executeShortSell = async () => {
                ...
                if (plan === 'personal' && userEmail) {
                    try {
                        ...
                        console.log('✅ Portfolio synced to backend');
                    } catch (error) {
            ...
            const loadStockNews = async () => {
                ...
                setLoadingNews(true);
                try {
                    console.log(`📰 Loading news for ${selectedStock}...`);
            ...
            const generateAIAnalysis = async () => {
                ...
                console.log('🧠 Generating AI analysis with REAL data for:', stock);
                ...
                const calculateEnhancedRSI = async () => {
                    try {
                        ...
                        console.log('⚠️ Using simplified RSI due to API error');
                    }
                ...
                console.log('📊 Real 14-day RSI calculated:', rsi.toFixed(1));
                ...
                const calculateMovingAverages = async () => {
                    try {
                        ...
                        console.log('⚠️ Could not fetch moving averages');
                    }
                ...
                console.log('📈 Real intraday volatility:', intradayVolatility.toFixed(2) + '%');
                ...
                console.log('📰 Real sentiment score:', sentimentScore, 'from', stockNews.length, 'articles');
                ...
                console.log('💡 Final recommendation:', overallRecommendation.action, 'at', overallRecommendation.confidence.toFixed(1) + '% confidence');
                ...
                console.log('✅ AI Analysis complete with REAL data:', analysis);
            ...
            const executeTrade = async (type) => {
                ...
                if (type === 'buy') {
                    ...
                    // SELL - Check position
                    console.log('🔍 Checking sell position for', selectedStock);
                    console.log('📊 All positions:', portfolio.positions);
                    const currentPosition = portfolio.positions[selectedStock] || 0;
                    console.log(`📈 ${selectedStock} position:`, currentPosition);
                    console.log('🔢 Trying to sell:', quantity);
            ...
            const generateDailySummary = () => {
                ...
                if (winRate >= 70 && totalProfitLoss > 0) {
                    ...
                    commentary.push(`⚖️ Win rate is positive at ${winRate.toFixed(1)}%, but P/L is negative. Consider letting winners run longer and cutting losers faster.`);
                ...
                if (todayTrades.length > 20) {
                    commentary.push("⚠️ Overtrading Alert: 20+ trades today increases emotional decision-making and commission costs. Focus on quality over quantity.");
                ...
                if (bestTrade && worstTrade && bestTrade.profit > 0 && worstTrade.profit < 0) {
                    ...
                    if (riskRewardRatio >= 2) {
                        ...
                        commentary.push(`⚠️ Risk-reward needs improvement: Best trade ${bestTrade.symbol} (+$${bestTrade.profit.toFixed(2)}) vs worst ${worstTrade.symbol} ($${worstTrade.profit.toFixed(2)}) = only ${riskRewardRatio.toFixed(1)}:1 ratio.`);
                    }
            ...
            const fetchHistoricalData = async (symbol, period = '1M') => {
                try {
                    console.log(`📊 Fetching real historical data for ${symbol} (${period})...`);
                    ...
                    if (data.s === 'ok' && data.t && data.c && data.t.length > 0) {
                        ...
                        console.log(`✅ Real historical data loaded for ${symbol} (${period}):`, {
                        ...
                        console.warn(`⚠️ No historical data available for ${symbol} from Finnhub`);
                        return null;
                    } else {
                        console.warn(`⚠️ Unexpected response from Finnhub for ${symbol}:`, data);
                        return null;
                    }
                } catch (error) {
            ...
            useEffect(() => {
                ...
                const loadChart = async () => {
                    ...
                    if (!historicalData) {
                        console.warn(`⚠️ Finnhub API unavailable - using simulated data for ${selectedStock}`);
                        ...
                        console.log(`✅ Real chart data loaded successfully from Finnhub for ${selectedStock}`);
            ...
            useEffect(() => {
                ...
                const stockNewsInterval = setInterval(() => {
                    console.log(`📰 Auto-refreshing news for ${selectedStock} (LIVE MODE)...`);
                    loadStockNews();
                }, 180000); // 3 minutes
            ...
            useEffect(() => {
                ...
                const newsRefreshInterval = setInterval(() => {
                    console.log('📰 Auto-refreshing market news (LIVE MODE)...');
                    loadMarketNews();
            ...
            if (view === 'code-display') {
                return (
                    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-700 to-blue-900 p-4 md:p-8 flex items-center justify-center">
                        ...
                        {(() => {
                            console.log('🎯 Checking modal render. showAccountTypeModal =', showAccountTypeModal);
                        ...
                        {showAccountTypeModal && (
                            <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 z-[9999]">
                                    <div className="relative bg-black/90 backdrop-blur-xl rounded-3xl p-8 max-w-6xl w-full border-2 border-cyan-500/40 shadow-2xl" style={{boxShadow: '0 0 60px rgba(6, 182, 212, 0.3)'}}>
                                        {/* Close Button */}
                                        <button
                                        ...
                                        <div className="text-center mb-8">
                                            <h2 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-green-400 to-cyan-300 mb-4" style={{textShadow: '0 0 60px rgba(6, 182, 212, 0.5), 0 0 30px rgba(74, 222, 128, 0.3)'}}>
                                    ...
                                    <div className="grid md:grid-cols-3 gap-6 pt-6">
                                        ...
                                        <button
                                            ...
                                            className="group relative bg-black/80 backdrop-blur-2xl rounded-3xl p-8 border-2 border-cyan-500/30 hover:border-cyan-400 transition-all duration-500 hover:-translate-y-3 hover:scale-105 overflow-hidden text-left"
                                            style={{boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)'}}
                                        >
                                            {/* Animated Background Gradient */}
                                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 via-blue-600/10 to-transparent opacity-0 group-hover:opacity-100 transition duration-700"></div>
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="flex items-center justify-center mb-6">
                                                    <div className="relative">
                                                        <div className="absolute inset-0 bg-cyan-500/30 blur-2xl rounded-full group-hover:bg-cyan-500/50 transition-all duration-500"></div>
                                                        <div className="relative bg-gradient-to-br from-cyan-400 via-blue-500 to-indigo-600 p-5 rounded-2xl shadow-xl transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                                                ...
                                                {/* Price */}
                                                <div className="relative text-center mb-8 py-6 bg-gradient-to-br from-black/60 to-black/40 rounded-2xl border-2 border-green-500/30 group-hover:border-green-400/60 transition-all duration-500 overflow-hidden">
                                        ...
                                        <button
                                            onClick={() => {
                                                if (accountMode === 'create') {
                                                    ...
                                                    if (STRIPE_FAMILY_PUBLIC_LINK.includes('REPLACE_WITH_YOUR_LINK')) {
                                                        alert('⚠️ Stripe payment link not configured yet!');
                                                        return;
                                                    }
                                            ...
                                            className="group relative bg-black/80 backdrop-blur-2xl rounded-3xl p-8 border-2 border-green-500/40 hover:border-green-400 transition-all duration-500 hover:-translate-y-3 hover:scale-105 overflow-hidden text-left"
                                        ...
                                        >
                                            ...
                                            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-20">
                                                <div className="relative">
                                                    ...
                                                    <span className="relative bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs font-black px-5 py-2 rounded-full shadow-2xl" style={{boxShadow: '0 0 30px rgba(34, 197, 94, 0.6)'}}>
                                                        ⭐ POPULAR
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <p className="text-center text-gray-400 group-hover:text-green-400 text-base mb-6 font-semibold transition-colors duration-300">Compete for prizes</p>

                                                {/* Price */}
                                                <div className="relative text-center mb-8 py-4 bg-gradient-to-br from-black/60 to-black/40 rounded-2xl border-2 border-yellow-500/30 group-hover:border-yellow-400/60 transition-all duration-500 overflow-hidden">
                                                    <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/5 to-orange-500/5 group-hover:from-yellow-500/10 group-hover:to-orange-500/10 transition-all duration-500"></div>
                                                    <div className="relative">
                                                        <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-orange-400 to-yellow-300" style={{textShadow: '0 0 40px rgba(251, 191, 36, 0.3)'}}>
                                                            1%
                                                        </span>
                                        ...
                                        <button
                                            ...
                                            className="group relative bg-black/80 backdrop-blur-2xl rounded-3xl p-8 border-2 border-purple-500/40 hover:border-purple-400 transition-all duration-500 hover:-translate-y-3 hover:scale-105 overflow-hidden text-left"
                                        ...
                                        >
                                            ...
                                            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-20">
                                                <div className="relative">
                                                    ...
                                                    <span className="relative bg-gradient-to-r from-purple-500 via-pink-500 to-purple-600 text-white text-xs font-black px-5 py-2 rounded-full shadow-2xl" style={{boxShadow: '0 0 30px rgba(168, 85, 247, 0.6)'}}>
                                                        ✨ PREMIUM
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                {/* Price */}
                                                <div className="relative text-center mb-8 py-4 bg-gradient-to-br from-black/60 to-black/40 rounded-2xl border-2 border-purple-500/30 group-hover:border-purple-400/60 transition-all duration-500 overflow-hidden">
                                                    ...
                                                    <div className="relative">
                                                        <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-400 to-purple-300" style={{textShadow: '0 0 40px rgba(168, 85, 247, 0.3)'}}>
                                                            $0.05
                                                        </span>
                        ...
                        <div className="min-h-screen bg-black relative overflow-hidden p-4 md:p-8">
                            ...
                            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                                ...
                                <button
                                    ...
                                    className="mb-6 text-cyan-400 hover:text-cyan-300 font-bold transition-all duration-300 flex items-center gap-2 group"
                                >
                                    <span className="group-hover:-translate-x-1 transition-transform duration-300">←</span><span>Back to Plans</span>
                                </button>

                                <div className="bg-black/80 backdrop-blur-2xl rounded-3xl p-8 md:p-12 border-2 border-cyan-500/30 shadow-2xl relative overflow-hidden" style={{boxShadow: '0 0 60px rgba(6, 182, 212, 0.2)'}}>
                                    {/* Create Account Form */}
                                    {accountMode === 'create' && (
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    try {
                                                        ...
                                                        console.log('📦 Portfolio loaded from backend:', account.portfolio);
                                                        console.log('📊 Positions:', account.portfolio?.positions);
                                                        console.log('💵 Cash:', account.portfolio?.cash);
                                                        ...
                                                        console.log('🎯 Competition state:', newCompetition);
                                                    } catch (error) {
                        ...
                        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
                            <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-900 rounded-3xl shadow-2xl max-w-2xl w-full border-2 border-purple-500 overflow-hidden animate-scaleIn max-h-[90vh] overflow-y-auto">
                                ...
                                <div className="p-6 bg-indigo-950 flex gap-3">
                                    <button onClick={() => setShowTradeAnalysis(false)} className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white px-6 py-3 rounded-xl font-bold transition-all">Got It!</button>
                        ...
                        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
                            <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-900 rounded-3xl shadow-2xl max-w-3xl w-full border-2 border-purple-500 overflow-hidden animate-scaleIn max-h-[90vh] overflow-y-auto">
                                ...
                                <div className="p-6 bg-indigo-950 sticky bottom-0">
                                    <button onClick={() => setShowDailySummary(false)} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white px-6 py-3 rounded-xl font-bold transition-all">Close</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div id="main-content" className="max-w-7xl mx-auto">
                        ...
                        <div className="mb-4 flex gap-3">
                            ...
                            <button
                                ...
                                className="group relative bg-black/80 backdrop-blur-xl hover:bg-cyan-500/20 border-2 border-cyan-500/30 hover:border-cyan-400 text-gray-300 hover:text-white px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2 overflow-hidden"
                            >
                                <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 bg-gradient-to-r from-transparent via-cyan-400/10 to-transparent"></div>
                            ...
                            <button
                                ...
                                className="group relative ml-auto bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2 shadow-xl hover:shadow-purple-500/30 overflow-hidden"
                            >
                        ...
                        {/* Main Tab Navigation */}
                        <div className="group/nav relative mb-6 bg-gradient-to-br from-black/95 via-black/90 to-black/95 backdrop-blur-3xl rounded-3xl p-3 border-2 border-cyan-500/40 hover:border-cyan-400/60 overflow-hidden transition-all duration-500" style={{boxShadow: '0 0 50px rgba(6, 182, 212, 0.3), inset 0 0 30px rgba(6, 182, 212, 0.05)'}}>
                            {/* Animated background gradient */}
                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 via-blue-600/5 to-purple-600/5 opacity-50"></div>

                            {/* Nav shine effect */}
                            <div className="absolute inset-0 -translate-x-full group-hover/nav:translate-x-full transition-transform duration-[1500ms] bg-gradient-to-r from-transparent via-cyan-400/15 to-transparent pointer-events-none"></div>

                            <div className="relative flex gap-3">
                                <button
                                    onClick={() => setMainTab('portfolio')}
                                    className={`group/tab relative flex-1 py-5 rounded-2xl font-black text-lg transition-all duration-500 overflow-hidden ${
                                        mainTab === 'portfolio'
                                            ? 'bg-gradient-to-br from-cyan-400 via-cyan-500 to-blue-600 text-white scale-105'
                                            : 'text-gray-400 hover:text-white hover:bg-gradient-to-br hover:from-cyan-500/30 hover:to-blue-600/30 hover:scale-102'
                                    }`}
                                ...
                                >
                                    {mainTab === 'portfolio' && (
                                        <>
                                            <div className="absolute inset-0 -translate-x-full group-hover/tab:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
                                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-400/20 to-blue-600/20 animate-pulse"></div>
                                        </>
                                    )}
                                    <span className="relative drop-shadow-lg">💼 Portfolio</span>
                                </button>
                                <button
                                ...
                                >
                                    ...
                                    <span className="relative drop-shadow-lg">📈 Trading</span>
                                </button>
                                <button
                                ...
                                >
                                    ...
                                    <span className="relative drop-shadow-lg">⭐ Watchlist</span>
                                </button>
                                <button
                                ...
                                >
                                    ...
                                    <span className="relative drop-shadow-lg">📰 News</span>
                                </button>
                                <button
                                    onClick={() => setMainTab('ai-analysis')}
                                    className={`group/tab relative flex-1 py-5 rounded-2xl font-black text-lg transition-all duration-500 overflow-hidden ${
                                        mainTab === 'ai-analysis'
                                            ? 'bg-gradient-to-br from-purple-500 via-purple-600 to-pink-600 text-white scale-105'
                                            : 'text-gray-400 hover:text-white hover:bg-gradient-to-br hover:from-purple-500/30 hover:to-pink-600/30 hover:scale-102'
                                    }`}
                                ...
                                >
                                    {mainTab === 'ai-analysis' && (
                                        <>
                                            <div className="absolute inset-0 -translate-x-full group-hover/tab:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
                                            <div className="absolute inset-0 bg-gradient-to-br from-purple-500/20 to-pink-600/20 animate-pulse"></div>
                                        </>
                                    )}
                                    <span className="relative drop-shadow-lg">🧠 AI Analysis</span>
                                </button>
                                <button
                                    onClick={() => {
                                        setMainTab('movers');
                                    ...
                                    className={`group/tab relative flex-1 py-5 rounded-2xl font-black text-lg transition-all duration-500 overflow-hidden ${
                                        mainTab === 'movers'
                                            ? 'bg-gradient-to-br from-orange-500 via-orange-600 to-red-600 text-white scale-105'
                                            : 'text-gray-400 hover:text-white hover:bg-gradient-to-br hover:from-orange-500/30 hover:to-red-600/30 hover:scale-102'
                                    }`}
                                ...
                                >
                                    ...
                                    <span className="relative drop-shadow-lg">🚀 Market Movers</span>
                                </button>
                            </div>
                        </div>

                        {/* Header */}
                        <div className="group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 border-cyan-500/30 hover:border-cyan-400/60 mb-6 overflow-hidden transition-all duration-500" style={{boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)'}}>
                            ...
                            <div className="relative flex justify-between items-center">
                                ...
                                {competition?.code && (
                                    <div className="relative bg-gradient-to-br from-black/60 to-black/40 rounded-2xl px-6 py-3 border-2 border-cyan-500/30 overflow-hidden group/code">
                                        <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/5 to-blue-500/5"></div>
                                        <div className="relative text-xs text-cyan-300 font-bold uppercase tracking-wider">Competition Code</div>
                                        <div className="relative text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 tracking-wider">{competition.code}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className={`group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 transition-all duration-500 hover:scale-105 overflow-hidden ${
                                portfolioReturn >= 0
                                    ? 'border-green-500/40 hover:border-green-400'
                                    : 'border-red-500/40 hover:border-red-400'
                            }`} style={{boxShadow: portfolioReturn >= 0 ? '0 0 30px rgba(34, 197, 94, 0.2)' : '0 0 30px rgba(239, 68, 68, 0.2)'}}>
                                <div className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-100 transition duration-700 ${
                                    portfolioReturn >= 0
                                        ? 'from-green-500/10 via-emerald-600/15 to-transparent'
                                        : 'from-red-500/10 via-pink-600/15 to-transparent'
                                }`}></div>
                                <div className={`absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent ${
                                ...
                                <div className={`relative text-sm font-bold mb-3 uppercase tracking-wide flex items-center gap-2 ${
                                    ...
                                    {portfolioReturn >= 0 ? '🚀' : '📉'} <span>Return</span>
                                </div>
                                <div className={`relative text-4xl font-black mb-2 text-transparent bg-clip-text bg-gradient-to-r ${
                                    portfolioReturn >= 0
                                        ? 'from-green-300 via-emerald-400 to-green-300'
                                        : 'from-red-300 via-pink-400 to-red-300'
                                }`} style={{textShadow: portfolioReturn >= 0 ? '0 0 20px rgba(74, 222, 128, 0.3)' : '0 0 20px rgba(239, 68, 68, 0.3)'}}>
                            ...
                            <div className="group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 border-purple-500/40 hover:border-purple-400 transition-all duration-500 hover:scale-105 overflow-hidden" style={{boxShadow: '0 0 30px rgba(168, 85, 247, 0.2)'}}>
                                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 via-pink-600/15 to-transparent opacity-0 group-hover:opacity-100 transition duration-700"></div>
                                <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-purple-400/15 to-transparent"></div>
                                <div className="relative">
                                    ...
                                    <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-400 to-purple-300">{watchlist.length}</div>
                                ...
                                {watchlist.length > 0 && (() => {
                                    ...
                                    watchlist.forEach(symbol => {
                                        ...
                                        if (change > 5) {
                                            alerts.push({ symbol, type: 'breakout', text: `${symbol} +${change.toFixed(1)}% 🚀`, color: 'green' });
                                        } else if (change < -5) {
                                            alerts.push({ symbol, type: 'dip', text: `${symbol} ${change.toFixed(1)}% 📉`, color: 'red' });
                                    ...
                                    return (
                                        <div className="mt-3 pt-3 border-t border-purple-500/30 text-[10px] text-purple-400">
                                            🔔 No alerts • Watching {watchlist.length} stocks
                                        </div>
                                    );
                                })()}
                            </div>
                        ...
                        {mainTab === 'portfolio' && (() => {
                            ...
                            if (portfolioReturn < -10) {
                                recommendations.push({ icon: '🛡️', text: 'High portfolio risk - review stop-loss positions', priority: 'high' });
                            }
                            ...
                            if (lossingPositions > profitablePositions && totalPositions > 0) {
                                recommendations.push({ icon: '⚠️', text: 'More losing than winning positions - review strategy', priority: 'high' });
                            }

                            return (
                                <div className="bg-gradient-to-br from-purple-900/60 via-indigo-900/60 to-purple-900/60 backdrop-blur-xl rounded-2xl p-6 border-2 border-purple-500/50 mb-6 shadow-2xl relative overflow-hidden">
                                    {/* Animated background */}
                                    ...
                                    <div className="relative">
                                        <div className="flex items-center justify-between mb-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-xs bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1 rounded-full font-bold">LIVE</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                                            ...
                                            {/* Win Rate */}
                                            <div className="bg-gradient-to-br from-blue-900/40 to-cyan-900/40 rounded-xl p-4 border-2 border-blue-500/50">
                                            ...
                                            <div className={`bg-gradient-to-br ${
                                                ...
                                                <div className={`text-2xl font-black ${
                                                    ...
                                                    {portfolioReturn > 5 ? '📈 Strong' : portfolioReturn < -5 ? '📉 Weak' : '➡️ Neutral'}
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="bg-gradient-to-br from-purple-900/60 via-indigo-900/60 to-blue-900/60 backdrop-blur-xl rounded-2xl p-8 border-2 border-purple-500/50 mb-6 shadow-2xl">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-300 to-cyan-300 flex items-center gap-3">
                                    ⚡ Daily Summary
                                    <span className="text-xs bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1 rounded-full font-bold">TODAY</span>
                                </h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {/* Today's Performance */}
                                <div className="bg-blue-950/50 backdrop-blur-sm rounded-xl p-6 border-2 border-cyan-500/30">
                                            ...
                                            <>
                                                ...
                                                <div className="text-sm text-cyan-200 mt-2">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span>📊 {tradesCount} trade{tradesCount !== 1 ? 's' : ''} today</span>
                                                    </div>
                                                    <div className="flex items-center gap-3 text-xs">
                                                        <span className="text-green-300">🔼 {buys} buy{buys !== 1 ? 's' : ''}</span>
                                                        <span className="text-blue-300">🔽 {sells} sell{sells !== 1 ? 's' : ''}</span>
                                                    </div>
                                                </div>
                                                <div className="mt-3 flex items-center gap-2">
                                                    <span className={`text-3xl ${isPositive && todayProfit > 0 ? 'animate-bounce' : ''}`}>
                                                        {isPositive && todayProfit > 0 ? '🎉' : todayProfit === 0 ? '💼' : '📉'}
                                                    </span>
                                ...
                                <div className="bg-blue-950/50 backdrop-blur-sm rounded-xl p-6 border-2 border-green-500/30">
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="text-2xl">🏆</span>
                                ...
                                <div className="bg-blue-950/50 backdrop-blur-sm rounded-xl p-6 border-2 border-red-500/30">
                                    <div className="flex items-center gap-2 mb-3">
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className="mt-6 bg-gradient-to-br from-purple-950/50 to-indigo-950/50 rounded-xl p-6 border-2 border-purple-500/30">
                                        <div className="flex items-center gap-3 mb-6">
                                            <div className="bg-gradient-to-br from-purple-500 to-pink-500 p-2 rounded-lg shadow-lg">
                                                <span className="text-2xl">🧠</span>
                                            </div>
                                            <div>
                                                <h3 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-pink-300">
                                                    Today's AI Trade Analysis
                                                </h3>
                                                <p className="text-sm text-purple-300">UltraThink deep reasoning • {todaysTrades.length} trade{todaysTrades.length !== 1 ? 's' : ''} analyzed</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4 max-h-96 overflow-y-auto custom-scrollbar">
                                            ...
                                            if (trade.type === 'buy') {
                                                if (priceChangePercent > 10) {
                                                    aiSummary = `🎯 EXCEPTIONAL ENTRY! ${trade.symbol} surged ${priceChangePercent.toFixed(2)}% since your buy at $${trade.price.toFixed(2)} (${holdText}). Strong bullish momentum. Target: $${(currentPrice * 1.15).toFixed(2)}. Stop-loss: $${(currentPrice * 0.93).toFixed(2)}. Consider scaling out 25-30% to lock gains while riding trend.`;
                                                    aiGrade = 'A++';
                                                    aiColor = 'text-green-400';
                                                } else if (priceChangePercent > 5) {
                                                    aiSummary = `✅ Excellent timing! ${trade.symbol} up ${priceChangePercent.toFixed(2)}% from $${trade.price.toFixed(2)}. Stock showing strength. Watch for breakout above $${(currentPrice * 1.05).toFixed(2)}. Trail stop at $${(currentPrice * 0.95).toFixed(2)} to protect profits. Bullish pattern forming.`;
                                                    aiGrade = 'A+';
                                                    aiColor = 'text-green-400';
                                                } else if (priceChangePercent > 2) {
                                                    aiSummary = `📊 Solid entry on ${trade.symbol}! Up ${priceChangePercent.toFixed(2)}% from $${trade.price.toFixed(2)}. Stock consolidating - healthy action. Resistance: $${(currentPrice * 1.08).toFixed(2)}. Support: $${(currentPrice * 0.96).toFixed(2)}. Good risk/reward setup.`;
                                                    aiGrade = 'A';
                                                    aiColor = 'text-green-300';
                                                } else if (priceChangePercent > 0) {
                                                    aiSummary = `🟢 Decent buy on ${trade.symbol}, +${priceChangePercent.toFixed(2)}% from $${trade.price.toFixed(2)}. Price holding above entry shows buyers. Watch $${(currentPrice * 1.03).toFixed(2)} breakout. If drops below $${(trade.price * 0.98).toFixed(2)}, reassess.`;
                                                    aiGrade = 'B+';
                                                    aiColor = 'text-green-300';
                                                } else if (priceChangePercent > -2) {
                                                    aiSummary = `⚠️ ${trade.symbol} down ${Math.abs(priceChangePercent).toFixed(2)}% from $${trade.price.toFixed(2)}. Normal volatility. Support at $${(trade.price * 0.97).toFixed(2)}. If holds, could bounce to $${(trade.price * 1.05).toFixed(2)}. Watch closely.`;
                                                    aiGrade = 'C+';
                                                    aiColor = 'text-yellow-400';
                                                } else if (priceChangePercent > -5) {
                                                    aiSummary = `🔴 ${trade.symbol} down ${Math.abs(priceChangePercent).toFixed(2)}% from $${trade.price.toFixed(2)}. Set stop-loss at $${(trade.price * 0.92).toFixed(2)} (max -8%). If breaks $${(currentPrice * 0.97).toFixed(2)}, expect drop to $${(currentPrice * 0.92).toFixed(2)}. Consider exit on bounce.`;
                                                    aiGrade = 'C-';
                                                    aiColor = 'text-orange-400';
                                                } else {
                                                    aiSummary = `🚨 URGENT: ${trade.symbol} down ${Math.abs(priceChangePercent).toFixed(2)}% from $${trade.price.toFixed(2)}! CUT LOSSES at market or wait for bounce to $${(currentPrice * 1.02).toFixed(2)}. Risk management critical. Review what went wrong!`;
                                                    aiGrade = 'D';
                                                ...
                                                if (gainLoss > 15) {
                                                    aiSummary = `🏆 MASTERFUL! ${trade.symbol} +${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}) selling at $${trade.price.toFixed(2)}. Now $${currentPrice.toFixed(2)} (${afterMove >= 0 ? '+' : ''}${afterMove.toFixed(1)}%). ${afterMove < -2 ? 'PERFECT timing! Avoided pullback.' : 'Excellent exit!'} Study this trade!`;
                                                    aiGrade = 'A++';
                                                    aiColor = 'text-green-400';
                                                } else if (gainLoss > 8) {
                                                    aiSummary = `🎉 Excellent! ${trade.symbol} +${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}) at $${trade.price.toFixed(2)}. Now $${currentPrice.toFixed(2)}. ${afterMove > 3 ? `Left ${afterMove.toFixed(1)}% more but profit is profit!` : 'Great timing!'} Disciplined execution.`;
                                                    aiGrade = 'A+';
                                                    aiColor = 'text-green-400';
                                                } else if (gainLoss > 3) {
                                                    aiSummary = `✅ Good profit on ${trade.symbol}: +${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}). Sold $${trade.price.toFixed(2)}, now $${currentPrice.toFixed(2)}. ${afterMove < -3 ? 'Dodged pullback!' : 'Solid trade.'} Small wins compound!`;
                                                    aiGrade = 'B+';
                                                    aiColor = 'text-green-300';
                                                } else if (gainLoss > 0) {
                                                    aiSummary = `🟢 Small win: ${trade.symbol} +${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}) at $${trade.price.toFixed(2)}. Better entry timing could improve results. Review why gain was limited.`;
                                                    aiGrade = 'B';
                                                    aiColor = 'text-green-300';
                                                } else if (gainLoss > -3) {
                                                    aiSummary = `⚪ Break-even: ${trade.symbol} ${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}) at $${trade.price.toFixed(2)}. Capital preserved. Analyze why it didn't work. No edge detected.`;
                                                    aiGrade = 'C';
                                                    aiColor = 'text-yellow-400';
                                                } else if (gainLoss > -8) {
                                                    aiSummary = `🔻 Loss: ${trade.symbol} ${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}) at $${trade.price.toFixed(2)}. Cutting losses = discipline! ${afterMove < -5 ? 'Saved from bigger loss!' : 'Review entry setup.'} Keep losses small!`;
                                                    aiGrade = 'C-';
                                                    aiColor = 'text-orange-400';
                                                } else {
                                                    aiSummary = `🚨 Big loss: ${trade.symbol} ${gainLoss.toFixed(2)}% (${dollarPL >= 0 ? '+' : ''}$${dollarPL.toFixed(2)}). REVIEW: Flawed thesis? Ignored stop? Size too big? ${afterMove < -5 ? 'At least you exited.' : ''} Learn, adapt, manage risk!`;
                                                    aiGrade = 'D';
                                                    aiColor = 'text-red-400';
                                                }
                                            }
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="text-xs text-gray-400 mt-1">
                                                                    {trade.quantity} shares @ ${trade.price.toFixed(2)} • {new Date(trade.timestamp).toLocaleDateString()}
                                                                </div>
                                                            </div>
                                                        </div>
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="bg-gradient-to-br from-indigo-900/60 via-purple-900/60 to-indigo-900/60 backdrop-blur-xl rounded-2xl p-6 border-2 border-indigo-500/50 shadow-2xl mb-6 relative overflow-hidden">
                                    {/* Animated background */}
                                    ...
                                    <div className="relative">
                                        <div className="flex items-center justify-between mb-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-purple-300">
                                                        AI Sector Analysis
                                                    </h2>
                                                    <p className="text-sm text-indigo-300">Real-time sector momentum • Market intelligence</p>
                                                </div>
                                            </div>
                                            <div className="text-xs bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-3 py-1 rounded-full font-bold">
                                                LIVE
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                            {topSectors.map((sector, idx) => {
                                                ...
                                                return (
                                                    ...
                                                    } rounded-xl p-4 border-2 hover:shadow-xl transition-all duration-300`}>
                                                        {/* Rank & Sector Name */}
                                                        <div className="flex items-start justify-between mb-3">
                                                            <div>
                                                                <div className="flex items-center gap-2 mb-1">
                                                        ...
                                                        <div className={`mb-3 px-3 py-2 rounded-lg text-center border ${
                                                            ...
                                                            <div className="text-xs font-bold text-white">
                                                                {sector.momentum === 'Strong Bullish' ? '🚀' :
                                                                 ...
                                                                 sector.momentum === 'Strong Bearish' ? '📉' :
                                                                 sector.momentum === 'Bearish' ? '⚠️' : '➡️'}
                                                                {' '}{sector.momentum}
                                                            </div>
                                                        </div>
                        ...
                        {/* Portfolio Positions */}
                        <div className="group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 border-cyan-500/30 hover:border-cyan-400/60 transition-all duration-500 overflow-hidden" style={{boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)'}}>
                            {/* Animated shine effect */}
                            <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-cyan-400/10 to-transparent"></div>
                                    ...
                                    Object.entries(portfolio.positions)
                                        ...
                                        .map(([symbol, qty]) => {
                                            ...
                                            return (
                                                <div
                                                    key={symbol}
                                                    className={`group/card relative transition-all duration-300 bg-black/80 backdrop-blur-xl border-2 rounded-xl p-4 hover:-translate-y-1 will-change-transform overflow-hidden ${
                                                        isPositive
                                                            ? 'border-green-500/40 hover:border-green-400'
                                                            : 'border-red-500/40 hover:border-red-400'
                                                    }`}
                                                    style={{boxShadow: isPositive ? '0 0 20px rgba(34, 197, 94, 0.15)' : '0 0 20px rgba(239, 68, 68, 0.15)'}}
                                                >
                                                    {/* Animated shine effect */}
                                                    <div className={`absolute inset-0 -translate-x-full group-hover/card:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent ${
                                                        isPositive ? 'via-green-400/10' : 'via-red-400/10'
                                                    } to-transparent`}></div>

                                                    <div className="relative flex justify-between items-start mb-3">
                                                        <div>
                                                            <div className="font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300 text-xl">{symbol}</div>
                                                            <div className="text-sm text-gray-400 font-semibold">{qty} shares</div>
                                                        </div>
                                                        <div className={`px-3 py-1 rounded-full font-bold text-sm transition-all ${
                                                            isPositive
                                                                ? 'bg-green-500/20 text-green-300 border border-green-500/50'
                                                                : 'bg-red-500/20 text-red-300 border border-red-500/50'
                                                        }`} style={{boxShadow: isPositive ? '0 0 15px rgba(34, 197, 94, 0.2)' : '0 0 15px rgba(239, 68, 68, 0.2)'}}>
                                                            {isPositive ? '↗' : '↘'} {isPositive ? '+' : ''}{gainLossPercent.toFixed(2)}%
                                                        </div>
                                                    </div>

                                                    <div className="relative grid grid-cols-2 gap-3 mb-3 text-sm">
                                                        <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-lg p-2">
                                                            <div className="text-cyan-300 text-xs font-semibold mb-1">Avg Price</div>
                                                            <div className="text-white font-bold">${avgBuyPrice.toFixed(2)}</div>
                                                        </div>
                                                        <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-lg p-2">
                                                            <div className="text-cyan-300 text-xs font-semibold mb-1">Current</div>
                                                            <div className="text-white font-bold">${stock.price.toFixed(2)}</div>
                                                        </div>
                                                    </div>

                                                    <div className={`relative flex justify-between items-end pt-3 border-t ${
                            ...
                            <div className="bg-gradient-to-br from-purple-800/40 to-indigo-900/40 rounded-2xl p-6 border-2 border-purple-500 mb-6 shadow-xl">
                                ...
                                <div className="mt-6 p-4 bg-purple-900/30 border border-purple-600/50 rounded-xl">
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-purple-200 font-semibold">💰 Total Prize Pool:</span>
                                        <span className="text-white font-black text-lg">${competition.totalPool?.toFixed(2) || '0.00'}</span>
                                    </div>
                                    {plan === 'family-public' && (
                                        <div className="text-xs text-purple-300 mt-2">1% fee per trade • Winner takes all</div>
                                    )}
                                    {plan === 'family-private' && (
                                        <div className="text-xs text-purple-300 mt-2">$0.05 flat fee per trade • Winner takes all</div>
                                    )}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="w-full">
                            ...
                            {(() => {
                                ...
                                stocks.forEach(stock => {
                                    ...
                                    if (change > 10) {
                                        score += 45;
                                        confidence += 25;
                                        reasons.push('🚀 Parabolic move - extreme momentum');
                                        type = 'STRONG BUY';
                                    } else if (change > 7) {
                                        score += 40;
                                        confidence += 22;
                                        reasons.push('🔥 Breakout confirmed - riding trend');
                                        type = 'STRONG BUY';
                                    } else if (change > 5) {
                                        score += 35;
                                        confidence += 20;
                                        reasons.push('📈 Strong uptrend - continuation likely');
                                        type = 'BUY';
                                    } else if (change > 3) {
                                        score += 28;
                                        confidence += 16;
                                        reasons.push('✅ Solid momentum building');
                                        type = 'BUY';
                                    } else if (change > 1.5) {
                                        score += 20;
                                        confidence += 12;
                                        reasons.push('📊 Positive trend forming');
                                        type = 'BUY';
                                    }
                                    ...
                                    if (change < -10) {
                                        score += 42;
                                        confidence += 20;
                                        reasons.push('💎 Extreme oversold - high rebound potential');
                                        type = 'BUY DIP';
                                    } else if (change < -7) {
                                        score += 35;
                                        confidence += 18;
                                        reasons.push('🎯 Deep discount - capitulation signal');
                                        type = 'BUY DIP';
                                    } else if (change < -5) {
                                        score += 28;
                                        confidence += 15;
                                        ...
                                        score += 22;
                                        confidence += 12;
                                        reasons.push('📉 Healthy correction - entry zone');
                                        type = 'BUY DIP';
                                    }
                                    ...
                                    if (relativeStrength > 5) {
                                        ...
                                        score += 18;
                                        confidence += 10;
                                        reasons.push('🏆 Leading the market');
                                    ...
                                    if (relativeStrength < -5 && change < 0) {
                                        score += 15;
                                        confidence += 10;
                                        reasons.push('🔄 Mean reversion candidate');
                                    }
                                    ...
                                    if (priceZone === 'micro' && change > 8) {
                                        score += 25;
                                        confidence += 12;
                                        reasons.push('🎲 Penny stock rocket - high risk/reward');
                                    } else if (priceZone === 'small' && change > 5) {
                                        score += 22;
                                        confidence += 15;
                                        reasons.push('💰 Small-cap growth momentum');
                                    } else if (priceZone === 'mid' && change > 3) {
                                        score += 20;
                                        confidence += 18;
                                        reasons.push('🏢 Mid-cap strength - sweet spot');
                                    } else if (priceZone === 'large' && change > 2) {
                                        score += 18;
                                        confidence += 20;
                                        reasons.push('🏛️ Blue-chip momentum - safer play');
                                    } else if (priceZone === 'mega' && change > 1.5) {
                                        score += 15;
                                        confidence += 22;
                                        reasons.push('👑 Mega-cap leader moving');
                                    }
                                    ...
                                    if (stockVolatility > 15) {
                                        ...
                                        confidence -= 5;
                                        reasons.push('⚠️ Extreme volatility - trader\'s paradise');
                                    } else if (stockVolatility > 10) {
                                        risk = 'HIGH';
                                        score += 10;
                                        confidence -= 3;
                                        reasons.push('⚡ High volatility - active trade');
                                    } else if (stockVolatility > 5) {
                                        risk = 'MEDIUM';
                                        score += 8;
                                        reasons.push('📊 Moderate volatility - good movement');
                                    } else if (stockVolatility > 2) {
                                        risk = 'LOW';
                                        score += 15;
                                        confidence += 5;
                                        reasons.push('🛡️ Stable growth - lower risk');
                                    } else {
                                        risk = 'VERY LOW';
                                        score -= 10; // Too quiet
                                        confidence -= 8;
                                        reasons.push('😴 Low activity - skip');
                                    }
                                    ...
                                    if (change >= 4 && change <= 7) {
                                        score += 25;
                                        confidence += 15;
                                        reasons.push('🎯 In the golden zone - optimal entry');
                                    }
                                    ...
                                    if (change < -5 && Math.abs(change) > 5) {
                                        score += 18;
                                        confidence += 10;
                                        reasons.push('🔄 V-recovery pattern forming');
                                    }
                                    ...
                                    if (change > 8 && change < 12) {
                                        score += 20;
                                        confidence += 12;
                                        reasons.push('💥 Breakout zone - momentum play');
                                    }
                                    ...
                                    if (marketSentiment === 'BULLISH' && change > 3) {
                                        score += 15;
                                        confidence += 8;
                                        reasons.push('🌊 Riding bullish market tide');
                                    } else if (marketSentiment === 'BEARISH' && change > 5) {
                                        score += 22;
                                        confidence += 12;
                                        reasons.push('🦅 Strong in weak market - leader');
                                    } else if (marketSentiment === 'BEARISH' && change < -8) {
                                        score += 18;
                                        confidence += 10;
                                        reasons.push('💰 Contrarian value play');
                                    }

                                    // ===== 7. PRICE LEVEL OPPORTUNITIES =====
                                    if (price < 5 && change > 10) {
                                        score += 20;
                                        reasons.push('🚨 Micro-cap moonshot potential');
                                    }
                                    if (price > 1000 && change > 2) {
                                        score += 12;
                                        confidence += 10;
                                        reasons.push('🏰 Mega-cap institutional strength');
                                    }
                                ...
                                return (
                                    <div className="bg-gradient-to-br from-purple-900/60 via-indigo-900/60 to-purple-900/60 backdrop-blur-xl rounded-2xl p-6 border-2 border-purple-500/50 shadow-2xl mb-6 relative overflow-hidden">
                                        {/* Animated background */}
                                        ...
                                        <div className="relative">
                                            <div className="flex items-center justify-between mb-6">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-purple-300">UltraThink AI recommendations • Live market analysis</p>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="text-xs bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1 rounded-full font-bold">
                                                        TOP {topPicks.length}
                                                    </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {topPicks.map((pick, idx) => {
                                                    ...
                                                    return (
                                                        <div key={idx} className="bg-gradient-to-br from-blue-900/40 to-indigo-900/40 rounded-xl p-4 border-2 border-blue-500/50 hover:border-cyan-400 hover:shadow-xl hover:shadow-cyan-500/20 transition-all duration-300 cursor-pointer group"
                                                            onClick={() => {
                                                            ...
                                                            <div className="flex items-start justify-between mb-3">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <div className={`text-xs font-bold px-2 py-1 rounded-full ${
                                                                        typeColor === 'emerald' ? 'bg-emerald-900/40 border border-emerald-400/70 text-emerald-300 animate-pulse' :
                                                            ...
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <div className="text-xs text-cyan-300 font-bold group-hover:scale-110 transition-transform">
                                                                    View Details →
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-purple-500/30 text-[10px] text-purple-300 text-center">
                                                ⚠️ AI recommendations are for educational purposes only. Past performance does not guarantee future results. Always do your own research.
                                            </div>
                                        </div>
                                    </div>
                                );
                            ...
                            <div className="group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 border-cyan-500/30 hover:border-cyan-400/60 transition-all duration-500 overflow-hidden" style={{boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)'}}>
                                ...
                                <h2 className="relative text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-blue-400 to-cyan-300 mb-4">🔍 Stock Search</h2>
                                ...
                                <div className="relative mb-6">
                                    ...
                                    {searching && (
                                        <div className="absolute right-4 top-4">
                                            <div className="text-cyan-300 animate-spin">🔍</div>
                                        </div>
                                    )}
                                </div>
                                ...
                                {searchQuery && showSearchResults && (
                                    <div className="group/results relative mb-6 bg-black/80 backdrop-blur-2xl border-2 border-cyan-500/30 rounded-xl overflow-hidden transition-all duration-500" style={{boxShadow: '0 0 30px rgba(6, 182, 212, 0.2)'}}>
                                        {/* Animated shine effect */}
                                ...
                                <h3 className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 mb-4 mt-8">📊 Select a Stock to Trade</h3>
                                {loading && stocks.length === 0 ? (
                                    <div className="text-center py-12 bg-black/80 backdrop-blur-2xl border-2 border-cyan-500/30 rounded-xl">
                            ...
                            {selectedStock && (
                                    <div className="group relative bg-black/80 backdrop-blur-2xl rounded-2xl p-6 border-2 border-cyan-500/30 hover:border-cyan-400/60 mt-6 overflow-hidden transition-all duration-500" style={{boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)'}}>
                                        {/* Animated shine effect */}
                                        <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-cyan-400/10 to-transparent pointer-events-none"></div>
                                        ...
                                        <div className="relative mb-6 pb-6 border-b border-cyan-500/30">
                                            <div className="flex items-center justify-between mb-3">
                                                <div className="flex items-center gap-3">
                                                    <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400">{selectedStock}</h2>
                                                    {/* AI Quick Signal */}
                                                    <div className="relative flex items-center gap-2 bg-gradient-to-r from-purple-500/20 to-indigo-500/20 border-2 border-purple-500/50 rounded-full px-3 py-1 overflow-hidden">
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <div className="mt-4 bg-purple-500/10 backdrop-blur-xl rounded-xl p-4 border-2 border-purple-500/40" style={{boxShadow: '0 0 20px rgba(168, 85, 247, 0.15)'}}>
                                                        <div className="flex items-center gap-2 mb-3">
                                                            <span className="text-lg">⏰</span>
                                            ...
                                            {!watchlist.includes(selectedStock) ? (
                                                ...
                                                <div className="mt-4 w-full bg-green-500/20 border-2 border-green-500/50 text-green-300 py-3 rounded-xl font-black text-lg flex items-center justify-center gap-2" style={{boxShadow: '0 0 20px rgba(34, 197, 94, 0.2)'}}>
                                                    <span>✓</span>
                                        ...
                                        {/* Price Display */}
                                        <div className="relative mb-6 text-center bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-xl p-6 border-2 border-cyan-500/40 overflow-hidden" style={{boxShadow: '0 0 30px rgba(6, 182, 212, 0.2)'}}>
                                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-blue-500/5"></div>
                                            <div className="relative text-sm text-cyan-400 font-black mb-2 uppercase tracking-wider">Current Price</div>
                                            <div className="relative text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 mb-1">
                                        ...
                                        {/* Price Chart */}
                                        <div className="relative mb-6 bg-black/60 backdrop-blur-xl rounded-xl p-4 border-2 border-cyan-500/30 overflow-hidden" style={{boxShadow: '0 0 25px rgba(6, 182, 212, 0.15)'}}>
                                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-blue-500/5"></div>
                                            ...
                                            {!isRealChartData && (
                                                <div className="relative mt-3 text-xs text-yellow-400 italic text-center font-semibold">
                                                    ⚠️ Using simulated historical data - Finnhub API temporarily unavailable
                                                </div>
                                            )}
                                        </div>
                                            ...
                                            {loadingAnalysis && (
                                                <div className="p-8">
                                                    <div className="text-center space-y-4">
                                                        ...
                                                        <div className="space-y-2">
                                                            <p className="text-purple-200 font-semibold animate-pulse">🧠 Initializing neural networks...</p>
                                                            <p className="text-purple-300 text-sm animate-pulse delay-100">📊 Analyzing technical indicators...</p>
                                                            <p className="text-purple-300 text-sm animate-pulse delay-200">📰 Processing news sentiment...</p>
                                                            <p className="text-purple-300 text-sm animate-pulse delay-300">⚡ Calculating risk factors...</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        <div className="mt-6 bg-gradient-to-br from-indigo-900/40 to-purple-900/40 rounded-2xl p-6 border-2 border-indigo-500 shadow-xl">
                                            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                                                <div>
                                                    ...
                                                    {lastStockNewsUpdate && (
                                                        <div className="text-xs text-indigo-300 mt-1 font-semibold">
                                                            🔄 Last updated: {lastStockNewsUpdate.toLocaleTimeString()} • Auto-refreshes every 3 min
                                                        </div>
                                                    )}
                                                </div>
                                                <button
                                                ...
                                                >
                                                    {loadingNews ? '⏳ Loading...' : '🔄 Refresh Now'}
                                                </button>
                                            </div>
                                                    ...
                                                    <div className="mb-4 p-3 bg-indigo-800/20 rounded-lg border border-indigo-600/30">
                                                        <div className="flex items-center justify-between text-sm">
                                                            <span className="text-indigo-200 font-semibold">
                                                                📊 {stockNews.length} {stockNews.length === 1 ? 'Article' : 'Articles'} Found
                                                            </span>
                                                ...
                                                {searching ? (
                                                    ...
                                                    searchResults.map((result) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="relative flex items-center justify-between">
                                                                <div className="flex-1">
                                                                    <div className="font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300 text-xl">{result.symbol}</div>
                                            ...
                                            {loadingAnalysis && (
                                                <div className="bg-gradient-to-br from-purple-900/60 to-indigo-950/60 backdrop-blur-xl rounded-2xl p-12 border-2 border-purple-500/50 shadow-2xl animate-pulse-glow-amber animate-scale-in">
                                                    <div className="text-center space-y-6">
                                                        ...
                                                        <div className="space-y-3">
                                                            <p className="text-purple-200 font-bold text-xl animate-pulse">🧠 Initializing neural networks...</p>
                                                            <p className="text-purple-300 animate-pulse">📊 Analyzing technical indicators...</p>
                                                            <p className="text-purple-300 animate-pulse">📰 Processing news sentiment...</p>
                                                            <p className="text-purple-300 animate-pulse">⚡ Calculating risk factors...</p>
                                                            <p className="text-purple-300 animate-pulse">🎯 Generating trade recommendations...</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                        ...
                        {mainTab === 'movers' && (
                            <>
                                <div className="bg-gradient-to-br from-orange-900/60 via-red-900/60 to-pink-900/60 rounded-2xl p-8 border-2 border-orange-500/50 shadow-2xl mb-6">
                                    <div className="text-center mb-8">
                                        <div className="inline-flex items-center justify-center w-24 h-24 bg-gradient-to-br from-orange-500 to-red-500 rounded-full mb-6 shadow-2xl">
                                            <span className="text-5xl">🚀</span>
                                        </div>
                                        <h2 className="text-4xl font-black text-white mb-3 flex items-center justify-center gap-3">
                                            Market Movers
                                            <span className="text-sm bg-gradient-to-r from-orange-500 to-red-500 text-white px-3 py-1 rounded-full font-bold">TODAY</span>
                                        </h2>
                                        ...
                                        <>
                                            ...
                                            <div className="mb-8">
                                                <div className="flex items-center gap-3 mb-6">
                                                    ...
                                                    <div>
                                                        <h3 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-green-300 to-emerald-300">
                                                            Top Gainers
                                                        </h3>
                                            ...
                                            <div>
                                                <div className="flex items-center gap-3 mb-6">
                                                    ...
                                                    <div>
                                                        <h3 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-300 to-pink-300">
                                                            Top Losers
                                                        </h3>
                    ...
                    <div className="relative max-w-7xl mx-auto px-4 py-12">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                            ...
                            <div className="group/section">
                                <h3 className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 font-black text-xl mb-4 flex items-center gap-2">
                                    <span className="text-2xl">💎</span>
                                    FinClash
                                </h3>
                            ...
                            <div className="group/section">
                                ...
                                <ul className="space-y-3">
                                    <li>
                                        <a href="privacy-policy.html" target="_blank" className="group/link relative text-gray-400 hover:text-cyan-300 text-sm transition-all font-semibold flex items-center gap-2">
                                            <span className="text-cyan-500/50 group-hover/link:text-cyan-400 transition-colors">→</span>
                                            <span>Privacy Policy</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="terms-of-service.html" target="_blank" className="group/link relative text-gray-400 hover:text-cyan-300 text-sm transition-all font-semibold flex items-center gap-2">
                                            <span className="text-cyan-500/50 group-hover/link:text-cyan-400 transition-colors">→</span>
                                            <span>Terms of Service</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="trust-safety.html" target="_blank" className="group/link relative text-gray-400 hover:text-cyan-300 text-sm transition-all font-semibold flex items-center gap-2">
                                            <span className="text-cyan-500/50 group-hover/link:text-cyan-400 transition-colors">→</span>
                            ...
                            <div className="group/section">
                                ...
                                <ul className="space-y-3">
                                    <li>
                                        <a href="about-us.html" target="_blank" className="group/link relative text-gray-400 hover:text-cyan-300 text-sm transition-all font-semibold flex items-center gap-2">
                                            <span className="text-cyan-500/50 group-hover/link:text-cyan-400 transition-colors">→</span>
                                            <span>About Us</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="contact.html" target="_blank" className="group/link relative text-gray-400 hover:text-cyan-300 text-sm transition-all font-semibold flex items-center gap-2">
                                            <span className="text-cyan-500/50 group-hover/link:text-cyan-400 transition-colors">→</span>
                        ...
                        <div className="border-t-2 border-cyan-500/20 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                            <p className="text-gray-500 text-sm font-semibold">
                                © 2025 <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 font-black">FinClash</span>. All rights reserved.
                            </p>
                            <p className="text-gray-500 text-sm font-semibold">
                                Created by <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 font-black">Reid Coleman</span> & <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-500 font-black">Cameron Dietzel</span>
                            </p>
                        </div>
                    </div>
                </footer>
